...
import { Session } from '@/components/modals/ReviewSessionModal';
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const handleNewChat = async () => {
    ...
    try {
      ...
      // Use the user's determined manager type, or default to CAPITALIST
      const selectedManagerType = user?.managerTypePreference || 'CAPITALIST';
    ...
    return directFetchedConversations
      ...
      .map((conversation) => (
        ...
        >
          ...
          <button
            onClick={(e) => handleReviewConversation(e, conversation)}
            className="ml-2 p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-all opacity-0 group-hover:opacity-70"
