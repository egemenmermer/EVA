...
export const MainLayout: React.FC<MainLayoutProps> = () => {
      ...
      if (details?.conversationId) {
        ...
        if (!currentConversation || currentConversation.conversationId !== details.conversationId) {
          ...
          const newConversation = {
            ...
            managerType: details.managerType || 'CAPITALIST',
  ...
  useEffect(() => {
    const handleConversationReview = (event: Event) => {
      const customEvent = event as CustomEvent;
      const session = customEvent.detail?.session;
  
      console.log("Conversation review event received:", session);
  
      if (session) {
        setReviewingSession(session);  // 👈 your modal state
      }
    };
  
    window.addEventListener("conversation-review", handleConversationReview as EventListener);
  
    return () => {
      window.removeEventListener("conversation-review", handleConversationReview as EventListener);
    };
  }, []);
